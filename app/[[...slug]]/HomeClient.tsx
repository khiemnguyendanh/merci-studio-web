...
const normalizeVestSizes = (input) => normalizeTextList(input)
    .map(size => String(size).replace(/^size[\s:_-]*/i, '').trim())
    .filter(Boolean);
...
const getVestSizes = (albumOrInput) => {
    if (!albumOrInput) return [];
    if (Array.isArray(albumOrInput) || typeof albumOrInput === 'string') return normalizeVestSizes(albumOrInput);

    const savedSizes = normalizeVestSizes(albumOrInput.sizes || []);
    const legacySizes = getAlbumHashtags(albumOrInput)
        .filter(tag => /^size[\s:_-]*/i.test(tag))
        .flatMap(tag => normalizeVestSizes(tag));

    return Array.from(new Set([...savedSizes, ...legacySizes]));
};
...
const albumMatchesVestSize = (album, size) => {
    if (!size) return true;
    return getVestSizes(album).some(albumSize => createSlug(albumSize) === createSlug(size));
};
...
export default function Home() {
    ...
    const [vestSizeFilter, setVestSizeFilter] = useState('');
    const [isCreatingAlbum, setIsCreatingAlbum] = useState(false);
    const [editingAlbum, setEditingAlbum] = useState(null);
    const [newAlbum, setNewAlbum] = useState({ title: '', sub: '', category: 'Wedding', sizes: '', hashtags: '', driveLink: '' });
    ...
    const [syncSizes, setSyncSizes] = useState('');
    ...
    const handleCreateAlbum = async () => {
        ...
        const vestSizes = createSlug(mainCategory) === 'vest' ? normalizeVestSizes(newAlbum.sizes || '') : [];
        const data = {
            id: `album_${Date.now()}`,
        ...
        setIsLoading(false);
        setNewAlbum({ title: '', sub: '', category: 'Wedding', sizes: '', hashtags: '', driveLink: '' });
    };
    ...
    const handleUpdateAlbum = async () => {
        ...
        try {
            ...
            const vestSizes = createSlug(mainCategory) === 'vest' ? normalizeVestSizes(editingAlbum.sizes || getVestSizes(editingAlbum)) : [];
    ...
    const handleSyncAlbumsFromDrive = async () => {
        ...
        const syncVestSizes = createSlug(mainCategory) === 'vest' ? normalizeVestSizes(syncSizes || '') : [];
                ...
                try {
                    ...
                    if (existingAlbum) {
                        await updateDoc(doc(db, 'merci_albums', existingAlbum.id), {
                            ...
                            sizes: getVestSizes(existingAlbum).length ? getVestSizes(existingAlbum) : syncVestSizes,
    ...
    const buildDriveMediaUrl = (fileId) => {
        if (!fileId) return '';
        if (process.env.NEXT_PUBLIC_CLOUDFLARE_PROXY) {
            return `${process.env.NEXT_PUBLIC_CLOUDFLARE_PROXY}?id=${encodeURIComponent(fileId)}`;
        }
    ...
    const albumHashtagFilters = Array.from(new Set(
        albums
            ...
            .flatMap(a => getAlbumHashtags(a))
            .filter(tag => !/^size[\s:_-]*/i.test(tag))
            .filter(Boolean)
    ));
    const vestSizeFilters = Array.from(new Set(
        albums
            .filter(a => albumMatchesCategory(a, 'Vest'))
    ...
    const navigateToVest = () => {
        setActiveTab('collection');
        setActiveAlbumId(null);
        setActiveBlogId(null);
        setActiveCategory('Vest');
        setAlbumHashtagQuery('');
        setVestSizeFilter('');
                ...
                <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4">
                    <div className="bg-white p-8 rounded-3xl w-full max-w-md space-y-6 shadow-2xl animate-in zoom-in-95">
                        ...
                        <div className="space-y-4">
                            ...
                            {createSlug(syncCategory) === 'vest' && (
                                <div>
                                    <label className="text-xs font-bold text-slate-500 ml-1">KÍCH CỠ VEST</label>
                                    <input
                                        type="text"
                                        placeholder="VD: 46, 48, 50, 52"
                                        ...
                                        value={syncSizes}
                                        onChange={e => setSyncSizes(e.target.value)}
                                    />
                                    <p className="text-xs text-slate-400 mt-1">Các size này sẽ áp dụng cho Vest mới được tạo từ Drive.</p>
                                </div>
                            )}
            ...
            {isCreatingAlbum && (
                <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4">
                    <div className="bg-white p-8 rounded-3xl w-full max-w-md space-y-6 shadow-2xl animate-in zoom-in-95">
                        ...
                        {createSlug(newAlbum.category) === 'vest' && (
                            <div>
                                <label className="text-xs font-bold text-slate-500 ml-1">KÍCH CỠ VEST</label>
                                <input
                                    type="text"
                                    ...
                                    value={newAlbum.sizes || ''}
                                    onChange={e => setNewAlbum({ ...newAlbum, sizes: e.target.value })}
                                />
                                <p className="text-[11px] text-slate-400 mt-1 ml-1">Nhập nhiều kích cỡ, cách nhau bằng dấu phẩy hoặc xuống dòng.</p>
                            </div>
                        )}
                        <div>
                ...
                <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 animate-in fade-in">
                    <div className="bg-white p-8 rounded-3xl w-full max-w-md space-y-6 shadow-2xl animate-in zoom-in-95">
                        ...
                        <div className="space-y-4">
                            ...
                            {createSlug(getAlbumMainCategory(editingAlbum, '')) === 'vest' && (
                                <div>
                                    <label className="text-xs font-bold text-slate-500 ml-1">KÍCH CỠ VEST</label>
                                    <input
                                        type="text"
                                        placeholder="VD: 46, 48, 50, 52"
                                        className="w-full border-2 border-slate-100 p-3 rounded-xl outline-none focus:border-blue-500 transition-colors mt-1"
                                        value={Array.isArray(editingAlbum.sizes) ? editingAlbum.sizes.join(', ') : (editingAlbum.sizes || getVestSizes(editingAlbum).join(', '))}
                                        onChange={e => setEditingAlbum({ ...editingAlbum, sizes: e.target.value })}
                                    />
                                    <p className="text-[11px] text-slate-400 mt-1 ml-1">Các size được dùng cho bộ lọc Vest ngoài website.</p>
                                </div>
                            )}
                            ...
                            {!activeAlbumId ? (
                                <>
                                    <div className="flex justify-between items-center">
                                        ...
                                        {isAdmin && (
                                            <div className="flex items-center gap-2">
                                                ...
                                                <button onClick={() => { setSyncCategory(activeCategory === 'Tất cả' ? 'Wedding' : activeCategory); setShowSyncModal(true); }} className="bg-emerald-600 text-white px-4 md:px-5 py-2 md:py-3 rounded-xl md:rounded-2xl font-bold flex items-center gap-2 shadow-lg active:scale-95 transition-all hover:bg-emerald-700 text-sm md:text-base">
                                                    <FolderDown size={18} /> <span className="hidden sm:inline">Đồng bộ Drive</span>
                                                </button>
                                                <button onClick={() => { setNewAlbum(prev => ({ ...prev, category: activeCategory === 'Tất cả' ? 'Wedding' : activeCategory })); setIsCreatingAlbum(true); }} className="bg-blue-600 text-white px-4 md:px-5 py-2 md:py-3 rounded-xl md:rounded-2xl font-bold flex items-center gap-2 shadow-lg active:scale-95 transition-all hover:bg-blue-700 text-sm md:text-base">
                                    ...
                                    {createSlug(activeCategory) === 'vest' && (
                                        <div className="bg-slate-900 text-white rounded-2xl p-4 md:p-5 mb-6 md:mb-8 shadow-lg">
                                            <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-5">
                                                <div className="shrink-0">
                                                    <p className="text-xs font-black text-white/50 uppercase tracking-widest">Lọc theo kích cỡ</p>
                                                    <p className="text-sm text-white/80 mt-1">Chọn một size Vest</p>
                                                </div>
                                                <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                                                    <button
                                                        onClick={() => setVestSizeFilter('')}
                                                        className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-all ${!vestSizeFilter ? 'bg-white text-slate-900' : 'bg-white/10 text-white hover:bg-white/20'}`}
                                                    >
                                                        Tất cả size
                                                    </button>
                                                    {vestSizeFilters.map(size => (
                                                        <button
                                                            key={size}
                                                            onClick={() => setVestSizeFilter(size)}
                                                            className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-all ${createSlug(vestSizeFilter) === createSlug(size) ? 'bg-white text-slate-900' : 'bg-white/10 text-white hover:bg-white/20'}`}
                                                        >
                                                            Size {size}
                                                        </button>
                                                    ))}
                                    ...
                                    {isAdmin && activeCategory !== 'Tất cả' && (
                                        <div className="bg-white border border-slate-100 rounded-2xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-3 shadow-sm">
                                            <div>
                                                ...
                                                <p className="font-mono text-sm text-blue-700 truncate">{createSlug(activeCategory) === 'vest' ? `${window.location.origin}/vest` : `${window.location.origin}/bo-su-tap${getCategoryHash(activeCategory)}`}</p>
                                            </div>
                                            <button onClick={() => {
                                                const link = createSlug(activeCategory) === 'vest' ? `${window.location.origin}/vest` : `${window.location.origin}/bo-su-tap${getCategoryHash(activeCategory)}`;
                                            ...
                                            >
                                                <div className="aspect-[4/5] rounded-2xl md:rounded-2xl overflow-hidden mb-2 md:mb-3 bg-slate-200 relative shadow-md group-hover:shadow-2xl transition-all duration-500">
                                                    ...
                                                    <div className="absolute top-2 md:top-6 left-2 md:left-6 z-10 flex flex-col items-start gap-1 max-w-[88%]">
                                                        ...
                                                        {createSlug(getAlbumMainCategory(a, '')) === 'vest' && getVestSizes(a).length > 0 && (
                                                            <span className="bg-slate-900/90 backdrop-blur-md px-2 md:px-3 py-0.5 md:py-1.5 rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-widest text-white shadow-sm">
                                                                Size {getVestSizes(a).join(' · ')}
                                                            </span>
                                                        )}
                                                    </div>
